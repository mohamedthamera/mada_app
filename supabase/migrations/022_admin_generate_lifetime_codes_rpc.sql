-- RPC fallback for admin code generation (when Edge Function returns 401 or is unavailable).
-- Callable by authenticated users; only admins (profiles.role = 'admin') can generate codes.
...
begin
  ...
  for i in 1 .. p_count loop
    -- Format similar to Edge Function: XXXX-XXXX-XXXX-XXXX (from random hex)
    v_code := upper(
